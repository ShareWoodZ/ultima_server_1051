	...
	if (str (_assignedRole) != str (r_player_lastSeat)) then {
		...
		if (count _assignedRole > 1) then {
			...
			_weapons = _vehicle weaponsTurret _turret;
			{
				...
				_action = _vehicle addAction [format["Зарядить %1",_weaponName], "\z\addons\dayz_code\actions\ammo.sqf",[_vehicle,_x,_turret], 0, false, true];
				r_player_actions2 set [count r_player_actions2,_action];
				r_action2 = true;
			} count _weapons;
		};
...
if (!isNull _menClose && _hasPatient && !r_drag_sqf && !r_action && !_inVehicle && !r_player_unconscious && _isClose) then {
	...
	if(_injured && _hasBandage) then {
		r_action = true;
		_action = _unit addAction [localize "str_actions_medical_04", "fixes\medical\bandage.sqf",[_unit], 0, true, true, "", ""];
		r_player_actions set [count r_player_actions,_action];
	};
	...
	if((_legsBroke || _armsBroke) && _hasMorphine) then {
		r_action = true;
		_action = _unit addAction [localize "str_actions_medical_06", "fixes\medical\morphine.sqf",[_unit], 0, true, true, "", ""];
		r_player_actions set [count r_player_actions,_action];
	};
	//Allow player to give Painkillers
	if(_inPain && _hasPainkillers) then {
		r_action = true;
		_action = _unit addAction [localize "str_actions_medical_07", "fixes\medical\painkiller.sqf",[_unit], 0, true, true, "", ""];
		r_player_actions set [count r_player_actions,_action];
	};
	//Allow player to transfuse blood
	if(_lowBlood && _hasBlood) then {
		r_action = true;
		_action = _unit addAction [localize "str_actions_medical_08", "fixes\medical\transfusion.sqf",[_unit], 0, true, true, "", ""];
		r_player_actions set [count r_player_actions,_action];
	};
