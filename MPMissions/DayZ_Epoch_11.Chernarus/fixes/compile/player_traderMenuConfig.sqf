...
TraderDialogLoadItemList	=
	{
		...
		_item_list	=	[];
		{
			private ["_header", "_item", "_name", "_type", "_textPart", "_qty", "_buy", "_bqty", "_bname", "_bax_in", "_bax_out", "_btype", "_sell", "_sqty", "_sname", "_stype", "_order", "_order", "_afile", "_File", "_count", "_bag", "_bagclass", "_index", "_image"];
			_header	=	_x select 0;
			_item	=	_x select 1;
			...
			switch (true) do
				{
					case (_type == 1):	{_type = "CfgMagazines";}; 
					case (_type == 2):	{_type = "CfgVehicles";}; 
					case (_type == 3):	{_type = "CfgWeapons";};
				};
			...
			switch(true)do
				{ 
					case (_btype == 1): {_btype = "CfgMagazines";}; 
					case (_btype == 2): {_btype = "CfgVehicles";}; 
					case (_btype == 3): {_btype = "CfgWeapons";}; 
				}; 
			_sell	=	_x select 4;
			...
			switch(true)do
				{
					case (_stype == 1):	{_stype = "CfgMagazines";}; 
					case (_stype == 2): {_stype = "CfgVehicles";}; 
					case (_stype == 3): {_stype = "CfgWeapons";}; 
				}; 
			
			switch (_bname) do
				{
					case "ItemSilverBar"		: {_bax_in	=	_bqty*Ultima_Bax_Silver};
					case "ItemSilverBar10oz"	: {_bax_in	=	_bqty*Ultima_Bax_Silver*10};
					case "ItemBriefcaseS100oz"	: {_bax_in	=	_bqty*Ultima_Bax_Silver*100};
					case "ItemGoldBar"			: {_bax_in	=	_bqty*Ultima_Bax_Silver*100};
					case "ItemGoldBar10oz"		: {_bax_in	=	_bqty*Ultima_Bax_Silver*1000};
					case "ItemBriefcase100oz"	: {_bax_in	=	_bqty*Ultima_Bax_Silver*10000};		
				};
			switch (_sname) do
				{
					case "ItemSilverBar"		: {_bax_out	=	_sqty*Ultima_Bax_Silver};
					case "ItemSilverBar10oz"	: {_bax_out	=	_sqty*Ultima_Bax_Silver*10};
					case "ItemBriefcaseS100oz"	: {_bax_out	=	_sqty*Ultima_Bax_Silver*100};
					case "ItemGoldBar"			: {_bax_out	=	_sqty*Ultima_Bax_Silver*100};
					case "ItemGoldBar10oz"		: {_bax_out	=	_sqty*Ultima_Bax_Silver*1000};
					case "ItemBriefcase100oz"	: {_bax_out	=	_sqty*Ultima_Bax_Silver*10000};		
				};
			
			_order	=	_x select 5;
			_afile	=	_x select 7;
			_File	=	"fixes\actions\" + _afile + ".sqf";
			_count	=	0;
			...
			_item_list set [count _item_list, [	_name,_textPart,_bax_in,_bname,"Бакса(ов)",_bax_out,_sname,"Бакса(ов)",_header,_File]];
...
TraderDialogShowPrices	=
	{
		...
		_index	=	_this select 0;
		if (_index < 0) exitWith {};
		while {count TraderItemList < 1} do 
			{sleep 1;};
