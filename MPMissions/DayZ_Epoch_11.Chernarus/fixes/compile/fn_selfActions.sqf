...
/*
	if (isnil ("s_player_debug_panel")) then	
		{
			player addaction[("<t color=""#0074E8"">" + ("Вкл\Выкл панель отладки") +"</t>"),"custom\playerstats.sqf","",5,false,true,"", ""];	
			s_player_debug_panel = 0;
		};
*/
...
if (DZE_HeliLift) then {
	...
	if(_inVehicle && (_vehicle isKindOf "Air") && ((getPos _vehicle select 2) < 30) && (speed _vehicle < 5) && (typeName _hasAttached == "OBJECT")) then {
...
if(DZE_HaloJump) then {
	if(_inVehicle && (_vehicle isKindOf "Air") && ((getPos _vehicle select 2) > 400)) then {
...
if (!isNull cursorTarget && !_inVehicle && !_isPZombie && (player distance cursorTarget < _allowedDistance) && _canDo) then {	//Has some kind of target
	...
	_player_lockUnlock_crtl = false;
	 ...
	 if (_canDo && (speed player <= 1) && (_cursorTarget isKindOf "Plastic_Pole_EP1_DZ")) then {
		 if (s_player_maintain_area < 0) then {
		  	s_player_maintain_area = player addAction [format["<t color='#ff0000'>%1</t>",localize "STR_EPOCH_ACTIONS_MAINTAREA"], "fixes\actions\maintain_area.sqf", "maintain", 5, false];
		 	s_player_maintain_area_preview = player addAction [format["<t color='#ff0000'>%1</t>",localize "STR_EPOCH_ACTIONS_MAINTPREV"], "fixes\actions\maintain_area.sqf", "preview", 5, false];
		 };
	 } else {
	...
	if (DZE_HeliLift) then {
		...
		if(_found && _allowTow && _canDo && !locked _cursorTarget && !_isPZombie && (typeName _attached != "OBJECT")) then {
			if (s_player_heli_lift < 0) then {
				s_player_heli_lift = player addAction ["Прицепить к вертолету", "\z\addons\dayz_code\actions\player_heliLift.sqf",[_liftHeli,_cursorTarget], -10, false, true, "",""];
			};
		} else {
	...
	if (_player_butcher) then {
		if (s_player_butcher < 0) then {
			if(_isZombie) then {
				s_player_butcher = player addAction [localize "STR_EPOCH_ACTIONS_GUTZOM", "fixes\actions\gather_zparts.sqf",_cursorTarget, 0, true, true, "", ""];
			} else {
	...
	if (_player_studybody) then {
		if (s_player_studybody < 0) then {
			s_player_studybody = player addAction [localize "str_action_studybody", "fixes\actions\study_body.sqf",_cursorTarget, 0, false, true, "",""];
		};
	} else {
	...
	if ((_cursorTarget isKindOf "ModularItems") || (_cursorTarget isKindOf "Land_DZE_WoodDoor_Base") || (_cursorTarget isKindOf "CinderWallDoor_DZ_Base")) then {
		...
		if (s_player_upgrade_build < 0) then {
			...
			s_player_upgrade_build = player addAction [format[localize "STR_EPOCH_ACTIONS_UPGRADE",_text], "fixes\actions\player_upgrade.sqf",_cursorTarget, -1, false, true, "",""];
		};
	} else {
		player removeAction s_player_upgrade_build;
		s_player_upgrade_build = -1;
...
//***************************************************Конец снятия одежды с трупа*********************************************************//

//***************************************************Шарим по карманам - ищем наличку*********************************************************//
	if (isNil "s_player_cash") then
		{s_player_cash	=	-1};
    if (_isMan && !_isAlive && !_isZombie && !_isAnimal && (speed player <= 1) ) then
		{
			if (s_player_cash < 0) then
				{s_player_cash = player addAction [("<t color='#0096ff'>")+("Обшмонать трупак")+("</t>"), "custom\cash.sqf",cursorTarget, -10, false, true, "",""];}
		}
	else
		{
...
//***************************************************Конец шарим по карманам - ищем наличку*********************************************************//
		
//***************************************************Забор крови из трупа*********************************************************//
	_BodyBloodBag = cursorTarget getVariable["BodyBloodBag",false];
	if (isNil "_BodyBloodBag") then
		{_BodyBloodBag	=	false;};
	if (isNil "s_player_BodyBloodBag") then
		{s_player_BodyBloodBag	=	-1};
     if (_isMan && !_isAlive && !_isZombie && !_BodyBloodBag && !_isAnimal && (speed player <= 1)) then
		{
			if (s_player_BodyBloodBag < 0) then
				{s_player_BodyBloodBag = player addAction [("<t color='#0096ff'>")+("Забор крови")+("</t>"), "custom\bodybloodbag.sqf",cursorTarget, -10, false, true, "",""];};
		}
	else
		{
...
//***************************************************Конец Забор крови из трупа*********************************************************//			
	...
	if ((dayz_myCursorTarget != _cursorTarget) && _isVehicle && !_isMan && _hasToolbox && (damage _cursorTarget < 1) && !_isDisallowRepair) then {
		if (s_player_repair_crtl < 0) then {
			...
			_menu = dayz_myCursorTarget addAction [localize "STR_EPOCH_PLAYER_REPAIRV", "fixes\actions\repair_vehicle.sqf",_cursorTarget, 0, true, false, "",""];
			_menu1 = dayz_myCursorTarget addAction [localize "STR_EPOCH_PLAYER_SALVAGEV", "fixes\actions\salvage_vehicle.sqf",_cursorTarget, 0, true, false, "",""];
		...
		if (s_player_parts_crtl < 0) then {
			...
			if(_humanity_logic) then {
				...
				// Static Menu
				{
					...
					_buy = player addAction [format["Обмен %1 %2 на %3 %4",(_x select 3),(_x select 5),(_x select 2),(_x select 6)], "fixes\actions\trade_items_wo_db.sqf",[(_x select 0),(_x select 1),(_x select 2),(_x select 3),(_x select 4),(_x select 5),(_x select 6)], (_x select 7), true, true, "",""];
				...
				s_player_parts set [count s_player_parts,_buy];
				if((_traderMenu select 2) == "miner") then 
					{
						// Add static metals trader options under sub menu
						_gems_trader = player addAction ["Торговля камнями", "custom\trade_gems.sqf",["na"], 0, true, false, "",""];
						s_player_parts set [count s_player_parts,_gems_trader];
					};					
...
//************************************************************ТОРГОВЫЙ ТЕРМИНАЛ************************************************************//
Private["_playerPos","_canTradeTerminal"];
_playerPos			=	getPosATL player;
_canTradeTerminal	=	count nearestObjects [_playerPos, ["Laptop_EP1"], 3] > 0;
if (isNil "s_player_trade_terminal") then
	{s_player_trade_terminal	=	-1};
if (isNil "s_player_transfer_terminal") then
	{s_player_transfer_terminal	=	-1};
if (isNil "s_player_change") then
	{s_player_change	=	-1};	
if (_canTradeTerminal) then
	{
        if (s_player_trade_terminal < 0) then
			{s_player_trade_terminal = player addaction[("<t color=""#0000c7"">" + ("Торговый терминал") +"</t>"),"custom\trade_terminal\sqf\terminal.sqf","",5,false,true,"", ""];};
        if (s_player_transfer_terminal < 0) then
			{s_player_transfer_terminal = player addaction[("<t color=""#0000c7"">" + ("Банковский терминал") +"</t>"),"custom\trade_terminal\sqf\transfer.sqf","",5,false,true,"", ""];};
        if (s_player_change < 0) then
			{s_player_change = player addaction[("<t color=""#0000c7"">" + ("Обменник") +"</t>"),"custom\trade_terminal\sqf\change.sqf","",5,false,true,"", ""];};			
    }
else
	{
        player removeAction s_player_trade_terminal;
        s_player_trade_terminal = -1;
        player removeAction s_player_transfer_terminal;
        s_player_transfer_terminal = -1;		
        player removeAction s_player_change;
        s_player_change = -1;		
    };
//*********************************************************ТОРГОВЫЙ ТЕРМИНАЛ-КОНЕЦ*********************************************************//

//************************************************************Установка ДШКМ.************************************************************//
Private["_playerPos","_canBaseBuilding","_base"];
_playerPos			=	getPosATL player;
_canBaseBuilding	=	false;
_base				=	nearestObjects [_playerPos, ["Plastic_Pole_EP1_DZ"], 30];
{
	if (dayz_characterID == (_x getVariable["CharacterID","0"]) ) exitWith
		{_canBaseBuilding	=	true};
} forEach _base;
if (isNil "s_player_dshkm") then
	{s_player_dshkm	=	-1};
if (_canBaseBuilding) then
	{
        if (s_player_dshkm < 0) then
			{s_player_dshkm = player addaction[("<t color=""#0000c7"">" + ("Установить пулемет") +"</t>"),"custom\base_buildings\dshkm.sqf","",5,false,true,"", ""];};
    }
else
	{
        player removeAction s_player_dshkm;
        s_player_dshkm = -1;
    };
//*********************************************************Конец. Установка ДШКМ.****************************************************//