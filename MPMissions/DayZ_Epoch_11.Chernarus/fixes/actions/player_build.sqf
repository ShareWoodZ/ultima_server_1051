...
if (_hasrequireditem) then
	{
		...
		if !(_classname in DZE_noRotate) then
			{
				s_player_setVectorsReset = player addaction ["Сброс наклона","custom\snap_pro\player_vectorChange.sqf","reset"];
				s_player_setVectorsForward = player addaction ["Наклонить ВПЕРЕД","custom\snap_pro\player_vectorChange.sqf","forward"];
				s_player_setVectorsBack = player addaction ["Наклонить НАЗАД","custom\snap_pro\player_vectorChange.sqf","back"];
				s_player_setVectorsLeft = player addaction ["Наклонить ВЛЕВО","custom\snap_pro\player_vectorChange.sqf","left"];
				s_player_setVectorsRight = player addaction ["Наклонить ВПРАВО","custom\snap_pro\player_vectorChange.sqf","right"];
				s_player_setVectors1 = player addaction ["Задать наклон 1 градус","custom\snap_pro\player_vectorChange.sqf","1"];
				s_player_setVectors5 = player addaction ["Задать наклон 5 градусов","custom\snap_pro\player_vectorChange.sqf","5"];
				s_player_setVectors45 = player addaction ["Задать наклон 45 градусов","custom\snap_pro\player_vectorChange.sqf","45"];
				s_player_setVectors90 = player addaction ["Задать наклон 90 градусов","custom\snap_pro\player_vectorChange.sqf","90"];
			};
		while {_isOk} do
			{
				...
				if(_location1 distance _location2 > 10) exitWith
					{
						_isOk = false;
						_cancel = true;
						_reason = "Вы отошли слишком далеко от места постройки (дальше 10 метров)";
						detach _object;
						deleteVehicle _object;
						detach _objectHelper;
						deleteVehicle _objectHelper;
				...
				if(_location1 distance _objectHelperPos > 10) exitWith
					{
						_isOk = false;
						_cancel = true;
						_reason = "Объект расположен слишком далеко от старта строительства (дальше 10 метров)";
						detach _object;
						deleteVehicle _object;
						detach _objectHelper;
						deleteVehicle _objectHelper;
				...
				if(abs(_objHDiff) > 10) exitWith
					{
						_isOk = false;
						_cancel = true;
						_reason = "Нельзя перемещать постройку, вверх или вниз, более чем на 10 метров.";
						detach _object;
						deleteVehicle _object;
						detach _objectHelper;
						deleteVehicle _objectHelper;
				...
				if (DZE_cancelBuilding) exitWith
					{
						_isOk = false;
						_cancel = true;
						_reason = "Строительство отменено.";
						detach _object;
						deleteVehicle _object;
						detach _objectHelper;
						deleteVehicle _objectHelper;
		...
		if (!DZE_BuildOnRoads) then
			{
				if (isOnRoad _position) then 
					{
						_cancel = true;
						_reason = "Невозможно установить на дороге.";
					};
			};
		if(!canbuild) then
			{
				_cancel = true;
				_reason = "Нельзя установить в трейд зоне.";
			};
		DC_noPlot=["Plastic_Pole_EP1_DZ"];
		if (_classname in DC_noPlot) then
			{
				_CorOffPlot = CoordOff_ZP;
				{
					_sector = (_x select 0);
					_pos = [(_x select 0),(_x select 1),0];
					_noPlots = (_x select 3);
					if (_noPlots >0) then
						{
							if ((player distance _pos) < _noPlots) then
								{
									_cancel = true;
									_reason = "На этой территории нельзя строить!";
								};		
						};
				} forEach _CorOffPlot;
			};
		if(!_cancel) then
			{
				...
				if (_classname == "MetalFloor_DZ") then
					{_buildOffset = [(_vUp select 0) * .148, (_vUp select 1) * .148,0]; };
				_location = 
					[
