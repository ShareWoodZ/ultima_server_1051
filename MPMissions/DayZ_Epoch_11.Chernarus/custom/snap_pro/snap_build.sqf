		...
		if (count s_player_toggleSnapSelectPoint != 0) then 
			{
				...
				s_player_toggleSnapSelectPoint=[]; snapActions = -1;
			};
		if (_s1 > 0) then
			{s_player_toggleSnap = player addaction [format[("<t color=""#ffffff"">" + ("Привязка: %1") +"</t>"),snapActionState],"custom\snap_pro\snap_build.sqf",[snapActionState,_object,_classname,_objectHelper],6,false,true];};
		if (_s2 > 0) then
			{s_player_toggleSnapSelect = player addaction [format[("<t color=""#ffffff"">" + ("Режим привязки: %1") +"</t>"),snapActionStateSelect],"custom\snap_pro\snap_build.sqf",[snapActionStateSelect,_object,_classname,_objectHelper],5,false,true];};
		if (_s3 > 0) then
			{
				s_player_toggleSnapSelectPoint=[];
				_cnt = 0;
				{
					snapActions = player addaction [format[("<t color=""#ffffff"">" + ("%1)Точка привязки: %2") +"</t>"),_cnt,_x select 3],"custom\snap_pro\snap_build.sqf",["Selected",_object,_classname,_objectHelper,_cnt],4,false,false];
...
fnc_initSnapPointsCleanup	=
	{
		...
		snapActionState = "Выкл";
	};

fnc_snapDistanceCheck	=
	{
		while {snapActionState != "Выкл"} do
			{
				...
				if (!isNull _distClosestPointFound) then
					{ 
						if (snapActionStateSelect == "Ручная") then
							{
		...
		if (isNil "snapTutorial") then
			{
				...
				if (_bldTxtEnable) then
					{
						_bldTxtStringTitle = format ["<t %1%2%3%4>Управление строительством</t><br />",_bldTxtClrO,_bldTxtSz,_bldTxtShdw,_bldTxtAlgnL,_bldTxtClrW,_bldTxtUndrln,_bldTxtBold];
						...
						_bldTxtStringSM = format["<t %1%3%4%5>[Режим привязки]<t %2> Ручная. Выберите нужный точку привязки.</t></t><br /><br />",_bldTxtClrC,_bldTxtClrW,_bldTxtSzT,_bldTxtShdw,_bldTxtAlgnL];
						_bldTxtStringPG = format["<t %1%3%4%5>[PgUP / PgDOWN]<t %2>: Регулировка высоты объекта на 10см</t></t><br />",_bldTxtClrC,_bldTxtClrW,_bldTxtSzT,_bldTxtShdw,_bldTxtAlgnL];
						_bldTxtStringAPG = format["<t %1%3%4%5>[Alt]+[PgUP / PgDOWN]<t %2>: Регулировка высоты объекта на 1м</t></t><br />",_bldTxtClrC,_bldTxtClrW,_bldTxtSzT,_bldTxtShdw,_bldTxtAlgnL];
						_bldTxtStringCPG = format["<t %1%3%4%5>[Ctrl]+[PgUP / PgDOWN]<t %2>: Регулировка высоты объекта на 1см</t></t><br />",_bldTxtClrC,_bldTxtClrW,_bldTxtSzT,_bldTxtShdw,_bldTxtAlgnL];
						...
						_bldTxtStringQEF = format["<t %1%3%4%5>[Q / E]<t %2>: Поворачивает объект на 45 градусов.</t></t><br /><br />",_bldTxtClrC,_bldTxtClrW,_bldTxtSzT,_bldTxtShdw,_bldTxtAlgnL];
						_bldTxtStringFD = format["<t %1%3%4%5>[F]<t %2>: Отлепить / Прилепить объект.</t></t><br />",_bldTxtClrO,_bldTxtClrW,_bldTxtSzT,_bldTxtShdw,_bldTxtAlgnL];
						...
						switch (_bldTxtSwitch) do
							{
								case "init"		:	{_bldTxtFinal = _bldTxtStringTitle + _bldTxtStringSD + _bldTxtStringPG + _bldTxtStringAPG + _bldTxtStringCPG + _bldTxtStringQE + _bldTxtStringQEF + _bldTxtStringFD;};
								case "OnAuto"	:	{_bldTxtFinal = _bldTxtStringTitle + _bldTxtStringSE + _bldTxtStringSA + _bldTxtStringPG + _bldTxtStringAPG + _bldTxtStringCPG + _bldTxtStringQE + _bldTxtStringQEF + _bldTxtStringFS;};
								case "Ручная"	:	{_bldTxtFinal = _bldTxtStringTitle + _bldTxtStringSE + _bldTxtStringSM + _bldTxtStringPG + _bldTxtStringAPG + _bldTxtStringCPG + _bldTxtStringQE + _bldTxtStringQEF + _bldTxtStringFS;};
							};
						[
...
switch (snapActionState) do
	{
		...
		case "Auto"	:
			{
				["Ручная",true] call fnc_initSnapTutorial;
				snapActionState = "Вкл";
				snapActionStateSelect = "Ручная";
