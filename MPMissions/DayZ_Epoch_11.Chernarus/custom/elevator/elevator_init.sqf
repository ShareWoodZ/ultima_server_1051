...
if (isServer || isDedicated) exitWith 
	{diag_log "ЛИФТ. ОШИБКА: Скрипт не может быть запущен с сервера";};
if (count _this != 1) exitWith 
	{diag_log "ЛИФТ. ОШИБКА: elevator_init.sqf вызван с неправильным количеством параметров (the only parameter should be the folder of the script relative to the mission folder)";};

_folder = (_this select 0) + "\";

// global variables
if (isNil "ELE_PlatformClass") then 
	{ ELE_PlatformClass = "MetalFloor_DZ" };
if (isNil "ELE_StopClass") then 
	{ ELE_StopClass = "MetalFloor_Preview_DZ" };
if (isNil "ELE_MaxRange") then 
	{ ELE_MaxRange = 25 }; // m
if (isNil "ELE_Size") then 
	{ ELE_Size = 4 }; // m
if (isNil "ELE_Speed") then 
	{ ELE_Speed = 2 }; // m/s
if (isNil "ELE_StopWaitTime") then 
	{ ELE_StopWaitTime = 5 }; // s
if (isNil "ELE_UpdatesPerSecond") then 
	{ ELE_UpdatesPerSecond = 60 }; // animation updates per second
if (isNil "ELE_RequiredBuildTools") then 
	{ ELE_RequiredBuildTools = ["ItemToolbox", "ItemCrowbar"] }; // required tools for building an elevator and elevator stop
if (isNil "ELE_RequiredBuildItems") then 
	{ ELE_RequiredBuildItems = [["PartGeneric",4], "PartEngine", "ItemGenerator", "ItemJerrycan"] }; // required items to build an elevator
if (isNil "ELE_RequiredBuildStopItems") then 
	{ ELE_RequiredBuildStopItems = [["PartGeneric",4]] }; // required items to build an elevator stop
if (isNil "ELE_Debug") then 
		...
		if ((!isNull _ct) && {(player distance _ct) < ELE_Size}) then
			{
						...
						if ([_ct] call ELE_fnc_isElevator) then
							{
								if (s_player_elevator_next < 0 && {[_ct] call ELE_fnc_hasNextStop}) then 
									{s_player_elevator_next = player addAction ["<t color=""#ffffff"">Активация лифта: Следующая остановка</t>", _folder+"elevator_actions.sqf", ["next",_ct], 5, false];};
								if (s_player_elevator_previous < 0 && {[_ct] call ELE_fnc_hasPreviousStop}) then
									{s_player_elevator_previous = player addAction ["<t color=""#ffffff"">Активация лифта: Предыдущая остановка</t>", _folder+"elevator_actions.sqf", ["previous",_ct], 5, false];};
						...
						else
							{
								if (s_player_elevator_upgrade < 0) then 
									{s_player_elevator_upgrade = player addAction ["<t color=""#ffffff"">Изготовить лифт</t>", _folder+"elevator_build.sqf", ["build",_ct], 0, false];};
								if (s_player_elevator_upgrade_stop < 0) then
									{s_player_elevator_upgrade_stop = player addAction ["<t color=""#ffffff"">Изготовить точку остановки лифта</t>", _folder+"elevator_build.sqf", ["build_stop",_ct], 0, false];};
							};
					};
				...
				if (s_player_elevator_id < 0 && ELE_Debug) then
					{s_player_elevator_id = player addAction ["<t color=""#ddffffff"">Показать идентификатор лифта</t>", _folder+"elevator_actions.sqf", ["id",_ct], 0, false];};
			}
		else
			{
