...
if (!isNil "AC_functions") then
	{
		if (AC_functions >= _version) then 
			{
				diag_log format ["ЛИФТ. ВНИМАНИЕ: Выполнение ac_functions.sqf пропущено, текущая версия %1, новее или такая же (версия %2)", AC_functions, _version];
				_skip = true;
			};
	};
if (_skip) exitWith {};
...
diag_log format ["AC_functions Версия %1", AC_functions];
AC_fnc_isArray	=
	{
...
AC_fnc_swapObject	=
	{
		...
		_objectCharacterID	=	
			if (count _this >= 2) then 
				[{_this select 1}, {_obj getVariable ["CharacterID","0"]}];
...
AC_fnc_hasTools = {
	...
	_hasTools = true;
	{
		if (!(_x in _items)) exitWith {
			...
			cutText [format["Нехватает инструмента: %1", _missing] , "PLAIN DOWN"];
		};
	} forEach _tools;
	_hasTools
};
...
AC_fnc_checkRequirements = {
	...
	_hasItems = true;
	{
		...
		if (_qty < _countIn) exitWith {
			...
			cutText [format["Нехватает %1 из %2", _missingQty, _textMissing], "PLAIN DOWN"];
		};
	} forEach _items;
	_hasItems
};
...
AC_fnc_removeRequirements = {
	...
	cutText [format["Нехватающие части, после первой проверки предмета: %1 / %2", _removed_total, _tobe_removed_total], "PLAIN DOWN"];
	false
};
