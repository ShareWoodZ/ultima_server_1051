...
if (isNull _character) exitWith
	{diag_log ("[Сервер]: Неудачная попытка синхронизации. Выход. Игрок обнулен: " + str(_character));};
...
if (_character isKindOf "Animal") exitWith
	{diag_log ("[Сервер]: Ошибка: Невозможно синхронизировать игрока: " + (_name) + ", класс: Животное");};
if (isnil "_characterID") exitWith
	{diag_log ("[Сервер]: Ошибка: Невозможно синхронизировать игрока: " + (_name) + ", обнулен characterID");};
if (_characterID == "0") exitWith
	{diag_log ("[Сервер]: Ошибка: Невозможно синхронизировать игрока: " + (_name) + ", отсутствует characterID");};
private["_debug","_distance"];
_debug	=	getMarkerpos "respawn_west";
_distance = _debug distance _charPos;
if (_distance < 2000) exitWith
	{diag_log format["[Сервер]: Ошибка: Невозможно синхронизировать игрока: %1 [%2]. Позиция в отладке: %3",_name,_characterID,_charPos];};
