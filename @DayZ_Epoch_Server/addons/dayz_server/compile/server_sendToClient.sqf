private ["_unit","_variable","_arraytosend","_owner","_vehicle","_qty","_karma"];
_unit	=	_this select 0;
if(isNull _unit) exitWith
	{diag_log format ["[Сервер]: Ошибка: Пустой пакет sendToClient: %1", _unit]};
...
switch (_variable) do
	{
		...
		case "Humanity":
			{
				_karma	=	(_arraytosend select 1);
				if ((_karma > 5) || (_karma < -5)) then
					{diag_log(format["[#Ultima]: [Карма]: Игроку %1, начислено %2 кармы, за убийство игрока.",_unit,_karma]);};
