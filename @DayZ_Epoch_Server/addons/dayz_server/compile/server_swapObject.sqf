...
if (!_allowed || !_proceed) exitWith
	{
		...
		diag_log ("[Сервер]: Неправильный объект, изменен playerUID: "+ str(_activatingplayerUID));
	};
...
diag_log ("[Сервер]: Публичное: " + str(_activatingPlayer) + " усовершенствовал " + (_class) + " с ID " + str(_uid));