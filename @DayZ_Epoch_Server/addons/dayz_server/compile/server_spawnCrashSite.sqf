...
diag_log("[Сервер]: Создание крушений: Запуск логики создания крушений.");
while {1 == 1} do
	{
		...
		diag_log(format["[Сервер]: Создание крушений: '%3', шанс создания %1%2,  из таблицы создания лута: '%4', за %5 секунд", round(_spawnChance * 100), '%', _crashName, _lootTable, _timeToSpawn]);
		...
		if (_spawnRoll <= _spawnChance) then
			{
				...
				diag_log(format["[Сервер]: Создание крушений: Создание: '%1' из таблицы создания лута: '%2'. СЕЙЧАС! (%3) на позиции: %4", _crashName, _lootTable, time, str(_position)]);
				...
				for "_x" from 1 to _num do
					{
						...
						diag_log(format["[Сервер]: Создание крушений: Создание лута: '%1' из таблицы создания лута: '%2'", _crashName, _lootTable]);
