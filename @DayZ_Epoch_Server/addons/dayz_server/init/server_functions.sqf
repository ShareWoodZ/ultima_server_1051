...
[] call compile preprocessFileLineNumbers "\z\addons\dayz_server\missions\DayZ_Epoch_11.Chernarus\custom\variables.sqf";	//	Переменные
[] call compile preprocessFileLineNumbers (Ultima_Server_Scripts_Path + "terminal\init.sqf");								//	Терминал торговли
...
server_removeWME			=	compile preprocessFileLineNumbers "\z\addons\dayz_server\compile\server_removeWME.sqf";	//	Удаление ПНВ, вооружения, патронов, для определенной техники
...
zombie_findOwner	=
	{
		...
		diag_log ("[Сервер]: Очистка: Удаление неконтролируемых зомби: " + (typeOf _unit) + " из: " + str(_unit) );
		#endif
		deleteVehicle _unit;
	};
...
spawn_vehicles =	
	{
		...
		if (count AllowedVehiclesList == 0) then
			{diag_log("[Сервер]: Отладка: Неудается найти подходящую для создания технику.");}
		else
			{
				...
				if ((count _position) == 2) then
					{
						...
						if((count _istoomany) > 0) exitWith
							{ diag_log("[Сервер]: Отладка: Слишком много техники на позиции: " + str(_position)); };
				...
				if ((count _position) == 2) then
					{
						...
						if((count _istoomany) > 0) exitWith
							{ diag_log("[Сервер]: Отладка рудников: Слишком много на позиции: " + str(_position)); };
				...
				if ((count _position) == 2) then
					{
						_istoomany	=	_position nearObjects ["All",5];
						if((count _istoomany) > 0) exitWith
							{ diag_log("[Сервер]: Отладка: Слишком много на позиции: " + str(_position)); };
						waitUntil{!isNil "BIS_fnc_selectRandom"};
						_spawnveh	=	_WreckList call BIS_fnc_selectRandom;
						if(DZEdebug) then
							{
				...
				if ((count _position) == 2) then
					{
						...
						if((count _istoomany) > 0) exitWith
							{ diag_log("[Сервер]: Отладка: Рудники: Слишком много объектов на позиции: " + str(_position)); };
						if(isOnRoad _position) exitWith
							{ diag_log("[Сервер]: Отладка: Рудники: На дороге: " + str(_position)); };
...
dze_diag_fps	=
	{
		if(DZE_DiagVerbose) then
			{diag_log format["[Сервер]: Отладка: ФПС: %1 Объектов: %2 : Игроков: %3", diag_fps,(count (allMissionObjects "")),(playersNumber west)];}
		else
			{diag_log format["[Сервер]: Отладка: ФПС : %1", diag_fps];};
	};
generate_new_damage	=
	{
		private ["_damage"];
...
dayz_perform_purge_player	=
	{
		...
		diag_log ("[Сервер]: Очистка игроков: " + str(_this));	
		if(!isNull(_this)) then
			{
...
server_spawncleanDead	=
	{
		...
		if (_delQtyZ > 0 || _delQtyP > 0) then
			{
				_qty	=	count _allDead;
				diag_log (format["[Сервер]: Очистка: Удалено %1 игроков, и %2 зомби из %3 мертвых",_delQtyP,_delQtyZ,_qty]);
			};
	};
server_cleanupGroups	=
	{
		if (DZE_DYN_AntiStuck3rd > 3) then
			{
				DZE_DYN_GroupCleanup	=	nil;
				DZE_DYN_AntiStuck3rd		=	0;
			};
		if (!isNil "DZE_DYN_GroupCleanup") exitWith
			{DZE_DYN_AntiStuck3rd = DZE_DYN_AntiStuck3rd + 1;};
...
server_checkHackers	=
	{
		if (DZE_DYN_AntiStuck2nd > 3) then
			{
				DZE_DYN_HackerCheck	=	nil;
				DZE_DYN_AntiStuck2nd	=	0;
			};
		if (!isNil "DZE_DYN_HackerCheck") exitWith
			{  DZE_DYN_AntiStuck2nd = DZE_DYN_AntiStuck2nd + 1;};
			...
			if (!((isNil "_x") || {(isNull _x)})) then
				{
					if(vehicle _x != _x && !(vehicle _x in PVDZE_serverObjectMonitor) && (isPlayer _x)  && !((typeOf vehicle _x) in DZE_safeVehicle)) then
						{
							diag_log ("[Сервер]: Очистка: Убийство читера: " + (name _x) + " " + str(_x) + " в " + (typeOf vehicle _x));
		...
		if (_delQtyFP > 0) then
			{
				...
				diag_log (format["[Сервер]: Очистка: Удалено %1 костров из %2",_delQtyNull,_qty]);
			};
	};
server_spawnCleanLoot	=
	{
		private ["_created","_delQty","_nearby","_age","_keep","_qty","_missionObjs","_dateNow"];
		if (DZE_DYN_AntiStuck > 3) then 
			{
				DZE_DYN_cleanLoot	=	nil;
				DZE_DYN_AntiStuck	=	0;
			};
		if (!isNil "DZE_DYN_cleanLoot") exitWith
			{  DZE_DYN_AntiStuck = DZE_DYN_AntiStuck + 1;};
		...
		if (_delQty > 0) then
			{
				...
				diag_log (format["[Сервер]: Очистка: Удалено %1 точек с лутом из %2",_delQty,_qty]);
			};
		DZE_DYN_cleanLoot	=	nil;
	};
		...
		if (_delQtyAnimal > 0) then
			{
				...
				diag_log (format["[Сервер]: Очистка: Удалено %1 животных из %2",_delQtyAnimal,_qty]);
			};
	};
		...
		if (!isNull(_obj)) then
			{
				...
				diag_log format["[Сервер]: Сейф %5. ID: %1. UID:%2. Принадлежит %3(%4)", _objectID, _objectUID, (name _player), (getPlayerUID _player), _statusText];
			};
	};