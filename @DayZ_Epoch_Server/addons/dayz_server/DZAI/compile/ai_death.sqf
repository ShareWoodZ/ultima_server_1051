...
if (_groupIsEmpty) then
	{_unitGroup call DZAI_protectGroup};
...
call
	{
		...
		if (_groupIsEmpty) then
			{_nul = [_unitGroup,300] spawn DZAI_deleteGroupTimed;};
	};
if !(isNull _victim) then
	{
		...
		if (_unitGroup getVariable ["CombatModeBlue",false]) then
			{_unitGroup setCombatMode "RED"};
		_victim spawn DZAI_deathFlies;
		_victim setVariable ["bodyName",_victim getVariable ["bodyName","unknown"],true];
